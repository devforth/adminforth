...
<a href="https://github.com/devforth/adminforth" style="font-weight: bold; text-decoration: underline" target="_blank">Star us on GitHub</a> to support a project!`,
  ...
  menu: [

    {
      ...
      component: '@@/Dash.vue',
      path: '/dashboard',
      visible: (user: any) => user.dbUser.role === 'superadmin',
      // homepage: true,
      isStaticRoute: false,
      // meta:{
      //   title: 'Dashboard',
...
admin.discoverDatabases().then(async () => {
  ...
  const simpleUserEmail = 'adminforth+1';
  if (!await admin.resource('users').get([Filters.EQ('email', simpleUserEmail)])) {
    await admin.resource('users').create({
      email: simpleUserEmail,
      password_hash: await AdminForth.Utils.generatePasswordHash('adminforth+1'),
      role: 'user',
    });
  }
