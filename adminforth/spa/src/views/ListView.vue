        ...
        <thead class="text-xs text-gray-700 bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
          <tr>
            ...
            <th v-for="c in columnsListed" scope="col" class="px-6 py-3">
              <div  @click="onSortButtonClick(c.name)" class="flex items-center cursor-pointer">
                {{ c.label }}
             
                <div  :style = "{ 'color':ascArr.includes(c.name)?'green':descArr.includes(c.name)?'red':'currentColor'}" >
                  <svg v-if="ascArr.includes(c.name) || descArr.includes(c.name)" class="w-3 h-3 ms-1.5" :class="{'rotate-180':descArr.includes(c.name)}" viewBox="0 0 24 24">
                    <path
