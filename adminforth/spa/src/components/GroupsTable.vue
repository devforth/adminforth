<template>
  <div class="shadow-resourseFormShadow dark:shadow-darkResourseFormShadow sm:rounded-lg dark:shadow-2xl rounded-default">
    <div v-if="group.groupName" class="text-md font-semibold px-6 py-3 flex flex-1 items-center border-b dark:border-gray-600 text-gray-700 bg-lightFormHeading dark:bg-gray-700 dark:text-gray-400">
      {{ group.groupName }}
    </div>
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
      <thead class="text-xs text-gray-700 uppercase bg-lightFormHeading dark:bg-gray-700 dark:text-gray-400 block md:table-row-group">
        <tr>
          <th scope="col" class="px-6 py-3 hidden md:table-cell">Field</th>
          <th scope="col" class="px-6 py-3 w-5/6 hidden md:table-cell">Value</th>
        </tr>
      </thead>
      <tbody>
        <tr
        ...
        >
          <td class="px-6 py-4 sm:pb-0 whitespace-nowrap flex items-center block md:table-cell">
            {{ column.label }}
            <span :data-tooltip-target="`tooltip-show-${i}`" class="ml-1 relative inline-block">
              <IconExclamationCircleSolid
                v-if="column.required[mode]"
                class="w-4 h-4"
          ...
          <td class="px-6 py-4 whitespace-nowrap whitespace-pre-wrap relative block md:table-cell">
            <template v-if="column?.components?.[props.source]?.file">
              <component
                ...
                @update:value="props.setCurrentValue(column.name, $event)"
            ...
            <template v-else>
              <Dropdown
                ...
                @update:modelValue="props.setCurrentValue(column.name, $event)"
              />
              <Dropdown
                single
              ...
              <input
                ...
                placeholder="0"
                :value="currentValues[column.name]"
                @input="props.setCurrentValue(column.name, $event.target.value)"
              />
              <CustomDatePicker
                ...
                auto-hide
                @update:valueStart="props.setCurrentValue(column.name, $event)"
              />
              <input
              ...
              >
                <IconEyeSolid v-if="!unmasked[column.name]" class="w-6 h-6 text-gray-400" />
                <IconEyeSlashSolid v-else class="w-6 h-6 text-gray-400" />
              </button>
            </template>
            <div v-if="columnError(column) && validating" class="mt-1 text-xs text-red-500 dark:text-red-400">
              {{ columnError(column) }}
            </div>
            <div v-if="column.editingNote && column.editingNote[mode]" class="mt-1 text-xs text-gray-400 dark:text-gray-500">
